...
export function ConnectionFlowDiagram({ animated, idPrefix = "connection" }: { animated: boolean; idPrefix?: string }) {
  ...
  return (
    ...
    >
      ...
      <g className="fill-primary font-mono text-[11px] tracking-[0.12em]">
        <text x="340" y="61" textAnchor="middle">REQUEST →</text>
      </g>
      <g className="fill-text-tertiary font-mono text-[11px] tracking-[0.12em]">
      ...
      {nodes.map((node, index) => (
        <g key={node.label} transform={`translate(${node.x} 89)`}>
          ...
          <text x="28" y="21" className="fill-text-tertiary font-mono text-[11px] tracking-[0.14em]">{node.eyebrow}</text>
          <text x="16" y="43" className="fill-foreground text-[12px] font-semibold">{node.label}</text>
          <text x="16" y="57" className="fill-text-tertiary font-mono text-[11px]">{node.detail}</text>
        </g>
      ))}
    </DiagramCanvas>
  )
