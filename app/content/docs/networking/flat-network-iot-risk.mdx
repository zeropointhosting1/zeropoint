...
The practical fix is VLANs: put IoT devices on their own segment, separate from the devices you actually trust, and put firewall rules between the segments that default to deny. This is exactly the structure behind the [home network on this site](/homelab) — three segments, each with a specific, narrow purpose:
...
Three lines, and the entire risk model changes. A compromised smart plug can still phone home to whatever cloud service it was already talking to — but it has no route to the NAS, no route to a laptop, no route to anything worth taking.
