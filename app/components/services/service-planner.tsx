  ...
  return (
    <div className="grid gap-10 lg:grid-cols-[1fr_360px] lg:items-start">
      <div className="space-y-10">
        <fieldset>
          <legend className="font-mono text-[11px] tracking-[0.14em] text-text-tertiary uppercase">1 · What do you need help with?</legend>
                  ...
                  {offeringsFor(audience).map((service) => {
                    ...
                    return (
                      <button key={service.id} type="button" onClick={() => toggleService(service.id)} className={cn("rounded-xl border p-4 text-left transition-colors", active ? "border-primary/50 bg-primary/8" : "border-border bg-surface-raised hover:border-primary/25")}>
                        ...
                        <span className="mt-3 block font-mono text-[11px] font-medium text-primary">{formatServicePrice(service.pricing)}</span>
                      </button>
                    )
                  })}
                </div>
        ...
        <fieldset>
          <legend className="font-mono text-[11px] tracking-[0.14em] text-text-tertiary uppercase">2 · How should we work together?</legend>
        ...
        <fieldset>
          <legend className="font-mono text-[11px] tracking-[0.14em] text-text-tertiary uppercase">3 · What should the project accomplish?</legend>
        ...
        <label className="block">
          <span className="font-mono text-[11px] tracking-[0.14em] text-text-tertiary uppercase">4 · Anything else?</span>
        ...
        <fieldset>
          <legend className="font-mono text-[11px] tracking-[0.14em] text-text-tertiary uppercase">5 · Where should this go?</legend>
      ...
      <aside className="sticky top-24 overflow-hidden rounded-2xl border border-primary/25 bg-surface/90 shadow-[0_24px_70px_-38px_var(--accent-glow),inset_0_1px_0_oklch(1_0_0/8%)]">
        <div className="border-b border-border px-6 py-5"><p className="font-mono text-[11px] tracking-[0.14em] text-primary uppercase">Your project brief</p><p className="mt-1 text-xs text-text-tertiary">Updates as you make selections</p></div>
        <div className="space-y-5 px-6 py-5 text-sm">
          ...
          {notes.trim() && <div><p className="font-mono text-[11px] tracking-wider text-text-tertiary uppercase">Notes</p><p className="mt-2 whitespace-pre-wrap text-xs leading-relaxed text-text-secondary">{notes}</p></div>}
        </div>
        ...
        <div className="space-y-3 border-t border-border px-6 py-5">
          {sendStatus === "sent" ? (
            ...
            <>
              ...
              <p className="text-center text-[11px] leading-relaxed text-text-tertiary">Add your name and email above to send. Final pricing is confirmed after scope review.</p>
            </>
          )}
        </div>
      </aside>
