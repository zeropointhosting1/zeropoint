...
const SERVICES = [
  { icon: ClipboardList, title: "Plan My Lab", copy: "A focused consultation around goals, workloads, hardware, networking, storage, and architecture." },
  { icon: Wrench, title: "Build My Lab", copy: "Hands-on help designing and setting up the environment, Proxmox, networking, and selected services." },
...
function LabUnit({ unit, label, tone }: { unit: string; label: string; tone: string }) {
  return <div className="mb-1.5 grid grid-cols-[32px_1fr_auto] items-center gap-3 rounded-md border border-border bg-surface px-3 py-2.5 last:mb-0"><span className="font-mono text-[9px] text-text-tertiary">{unit}</span><span className="text-sm font-medium text-foreground">{label}</span><span className={`size-1.5 rounded-full bg-current ${tone}`} /></div>
}
