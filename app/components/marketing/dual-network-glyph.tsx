...
function DeviceCard({ x, y, label, zone, tone }: { x: number; y: number; label: string; zone: string; tone: Tone }) {
  ...
  return (
    <g transform={`translate(${x} ${y})`}>
      ...
      <text x="14" y="40" className={`${colors.text} font-mono text-[8px] tracking-wide uppercase`}>{zone}</text>
    </g>
  )
}
...
function AccessPointCard({ x, y, label, detail, animated }: { x: number; y: number; label: string; detail: string; animated: boolean }) {
  return (
    <g transform={`translate(${x - 62} ${y - 27})`}>
      ...
      <text x="14" y="44" className="fill-text-secondary font-mono text-[8px] tracking-wide uppercase">{detail}</text>
...
export function DualNetworkGlyph({ animated }: { animated: boolean }) {
  ...
  return (
    <DiagramCanvas viewBox="0 0 900 455" className="h-full w-full" ariaLabel="Live network preview showing devices connected beneath two access points and an isolated lab network">
      <text x="24" y="25" className="fill-text-secondary font-mono text-[9px] tracking-[0.16em] uppercase">Home network · live flow</text>
      ...
      <text x="860" y="25" textAnchor="end" className="fill-success font-mono text-[8px] tracking-[0.14em] uppercase">Online</text>

      <rect x="14" y="216" width="628" height="226" rx="16" className="fill-primary/3 stroke-primary/15" strokeDasharray="4 7" />
      <text x="30" y="238" className="fill-primary font-mono text-[8px] tracking-[0.14em] uppercase">Wireless coverage + client VLANs</text>
      ...
      <text x="678" y="238" className="fill-text-tertiary font-mono text-[8px] tracking-[0.14em] uppercase">Isolated boundary</text>
